...
//    $from_name_str = $env['from_name_str'];
    ...
    while (true) {
        ...
        // 检查是否为预热邮件
        if (strpos($task_email, '|||warm|||') !== false) {
            $parts = explode('|||warm|||', $task_email);
            $task_email = $parts[0];
            $designated_smtp = intval($parts[1]);
            $is_warm_flag = true;
        }
        // 如果是预热邮件，使用指定的SMTP
        if ($is_warm_flag && $designated_smtp >= 0) {
            $smtp_index = $designated_smtp;
        ...
        $smtp_key = 'smtp_' . $smtp_index;
        $smtp_last_send = $redis->get($smtp_key . '_last_send');
        $smtp_send_count = $redis->get($smtp_key . '_count');
        ...
        $from_name_index = $smtp_index % $redis->lLen('from-name-list');
        $from_name = $redis->lIndex('from-list',$from_name_index);
        ...
        try {
            ...
            $mailer->isHTML(true);
            $mailer->Subject = replacePlaceholders($title,$redis) ;
            $mailer->Body = replacePlaceholders($content,$redis);
            $mailer->send();
            $error_num = 0;
            $mailer->smtpClose();
            ...
            // 自动冷却机制
            $smtp_send_count = $redis->incr('smtp_send_count_' . $smtp_index);
            if ($env['enable_auto_cooling'] && $smtp_send_count >= $env['cooling_threshold']) {
                sleep($env['cooling_duration']); // 冷却等待
                $redis->del('smtp_send_count_' . $smtp_index); // 重置计数
            }

            // 添加自动预热检查
            if (!$is_warm_flag && $env['auto_warm_enabled']) {
                $current_sent = $redis->get('process') ?: 0;
                if ($current_sent > 0 && $current_sent % intval($env['auto_warm_interval']) == 0) {
                    executeWarmup($redis, $env);
                    file_put_contents('auto_warm.log',
                        date('Y-m-d H:i:s') . " - 自动预热触发，已发送: {$current_sent}封\n",
                        FILE_APPEND);
                }
            }
        } catch (Exception $e) {
